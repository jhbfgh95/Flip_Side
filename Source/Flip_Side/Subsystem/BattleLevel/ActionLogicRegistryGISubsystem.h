...
using FActionResolveLogic = TFunction<FWeaponActionResolveResult(class UActionBase* Context)>;
...
private:
	...
	TMap<int32, FActionResolveLogic> WeaponResolveMap;
	TMap<int, FActionLogic> ItemMap;
	
public:
	void RegisterWeaponLogic(int ID, FActionLogic Logic) { WeaponMap.Add(ID, Logic); }
	void RegisterWeaponResolveLogic(int ID, FActionResolveLogic Logic) { WeaponResolveMap.Add(ID, Logic); }
	...
	FActionResolveLogic GetWeaponResolveLogic(int ID) { return WeaponResolveMap.FindRef(ID); }
