...
#include "Subsystem/GridManagerSubsystem.h"
...
void UStageCardWSubsystem::RemoveHandCard(int32 HandIndex)
{
    ...
    if (World)
    {
        if (UGameInstance* GI = World->GetGameInstance())
        {
            ...
            {
                // 함수 시그니처: SetBattleCardID(CardID, Slot)
...
{
    // 1) 이전 계산 결과 제거 (이번 라운드/턴에서 새로 계산)
    ClearAllModifiers();
    ...
    for (int32 Slot = 0; Slot < HandCount; ++Slot)
    {
        ...
        if (CardID < 0) // -1은 빈 슬롯으로 쓰는 경우가 많아서
            continue;
        ...
        case 4:
        {
            // TODO: 골드 시스템/플레이어 상태에 AddGold 같은 함수 호출
            break;
        }
        // 5) One for All
        case 5:
        ...
        case 6:
        {
            ...
            for (const auto& Pair : ClassCount)
            {
                ...
                for (const FCoinOnGridInfo& Info : FieldCoins)
                {
                    ...
                    {
                        // TODO: MaxHP +1 같은 건 CoinActor에 적용 함수 만들거나, 전투 계산에서 "MaxHPAdd" 같은 modifier를 따로 두는 방식으로 하기
                    }
                    else
                        Mods.BehaviorAdd += 4;
                }
